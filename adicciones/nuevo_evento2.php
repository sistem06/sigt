...
<?php
  $tx_m = "select * from tb_institucion_participantes where (ip_ev_id = '".$_GET['ev_id']."')";
...
<div class="panel panel-danger">
  ...
  <h3 class="panel-title">No se ha agregado ninguna Institucion participante en este evento</h3></div>
</div>
<?php
  } else {
?>
<table class="table table-striped">
  <thead>
    <tr>
      <th>Institucion Participante</th>
  ...
  <tbody>
    ...
    while($ar_m = mysql_fetch_array($query_m)){
      ...
      echo '<tr>';
        ...
        echo '<td>'.DatoRegistro ('tb_instituciones_cpa', 'ins_name', 'ins_id',$ar_m['ip_ins_id'], $conn).'</td>';
        echo '<td><a href="tools/quitar.php?val='.$ar_m['ip_id'].'&id=ip_id&tabla=tb_institucion_participantes"  title="eliminar" class="fancybox fancybox.iframe"><button type="button" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-trash"></span></button></a></td>';
      echo '</tr>';
    }
    ?>
  </tbody>
...
<?php echo SelectGeneralAccion("ip_ins_id", "form-control", "org", "Institucion Participante:  ", "tb_instituciones_cpa", "ins_id", "ins_name", "Instituciones"); ?>
...
<input type="hidden" name="ip_ev_id" value="<?php echo $_GET['ev_id']; ?>">
