...
<?php
echo "Referentes de ".DatoRegistro ('tb_datos_personales', 'dp_name', 'dp_id', $_GET['dp_id'], $conn).' ('.DatoRegistro ('tb_datos_personales', 'dp_nro_doc', 'dp_id', $_GET['dp_id'], $conn).')';
?>
</span>

</div>
<?php
  $tx_m = "select * from tb_referentes_beneficiarios where (rb_dp_id = '".$_GET['dp_id']."')";
...
<div class="panel panel-danger">
  ...
  <h3 class="panel-title">No se ha agregado ningun Referente para este Beneficiario</h3></div>
</div>
<?php
  } else {
?>
<table class="table table-striped">
  ...
  <tbody>
    ...
    while($ar_m = mysql_fetch_array($query_m)){
      $par = DatoRegistro ('tb_datos_personales_referente', 'dpr_parentesco', 'dpr_id', $ar_m['rb_dpr_id'], $conn);
      echo '<tr>';
        echo '<td>'.DatoRegistro ('tb_datos_personales_referente', 'dpr_name', 'dpr_id', $ar_m['rb_dpr_id'], $conn).'</td>';
        echo '<td>'.DatoRegistro ('tb_datos_personales_referente', 'dpr_calle', 'dpr_id', $ar_m['rb_dpr_id'], $conn).' '.DatoRegistro ('tb_datos_personales_referente', 'dpr_nro', 'dpr_id', $ar_m['rb_dpr_id'], $conn).'</td>';
        echo '<td>'.DatoRegistro ('tb_datos_personales_referente', 'dpr_telefono', 'dpr_id', $ar_m['rb_dpr_id'], $conn).' '.DatoRegistro ('tb_datos_personales_referente', 'dpr_movil', 'dpr_id', $ar_m['rb_dpr_id'], $conn).'</td>';
        echo '<td>'.DatoRegistro ('tb_parentesco', 'par_name', 'par_id', $par, $conn).'</td>';
        echo '<td><a href="tools/quitar.php?val='.$ar_m['rb_id'].'&id=rb_id&tabla=tb_referentes_beneficiarios"  title="eliminar" class="fancybox fancybox.iframe"><button type="button" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-trash"></span></button></a></td>';
      echo '</tr>';
    }
    ?>
  </tbody>
...
<?php
  ...
  ?>
<button class="btn btn-warning" id="addfamiliar">Agregar Referente</button>
