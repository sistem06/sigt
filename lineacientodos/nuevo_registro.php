...
<form id="parte1" action="tools/add_registro.php" method="post" role="form" onkeypress="return pulsar(event)">
...
<div class="row">
   ...
   <?php echo SelectFiltroMenorNro("la_102_tipo_llamado", "form-control", "idtipollamado", "Tipo de llamado:","tb_motivo_llamado", "ml_id", "ml_name","5"); ?>
    ...
    <div class="col-xs-12 col-md-6">
  <div class="form-group has-success">
   <?php echo SelectGeneral("la_102_relacion", "form-control", "idtiporelacion", "Origen del llamado:","tb_relacion_danmificado", "rd_id", "rd_name"); ?>
</div>
   </div>
   </div>
<div id="datos_instituciones" style="display:none;">
    
  <div class="form-group">
    <label>Institucion:</label>
      <select name="list_instituciones" class="form-control" id="listinstituciones">
      </select>
</div>
<div id="agrega_instituciones" style="display:none;">

  <div class="form-group">
    <label> Nuava Institucion:</label>
      <input name="nueva_institucion" class="form-control" id="nuevainstitucion" type="text">
</div>


  <div class="form-group">
    
     <button id="agrega_ins" class="btn btn-warning" type="button">Agregar</button>
</div>
...
<div id="datos_quien_llama" style="display:none;">
    <div class="row">
  <div class="col-xs-12 col-md-4">
  <div class="form-group">
   <?php echo InputGeneral("text", "dp_102_apellido", "form-control", "idapellidollama", "escriba el apellido", "Apellido del que llama:",""); ?>
</div>
   </div>
   <div class="col-xs-12 col-md-4">
   <div class="form-group">
   <?php echo InputGeneral("text", "dp_102_nombre", "form-control", "idnombrellama", "escriba el nombre", "Nombre del que llama:",""); ?>
</div>
   </div>
   <div class="col-xs-12 col-md-4">
   <div class="form-group">
   <?php echo SelectGeneral("dp_102_parentesco", "form-control", "parentescollama", "Relacion con la victima:","tb_parentesco", "par_id", "par_name"); ?>
</div>
   </div>
   </div>
...
<div class="row">
  <div class="col-xs-12 col-md-4">
<div class="form-group">
   <?php echo InputGeneral("tel", "dp_102_tel1", "form-control", "nrotel_llamado", "escriba el nro de telefono", "Teléfono fijo:",""); ?>
  </div>
  </div>
<div class="col-xs-12 col-md-4">
  <div class="form-group">
   <?php echo InputGeneral("text", "dp_102_tel2", "form-control", "nrocelu_llamado", "escriba el nro de celular", "Teléfono Celular:",""); ?>
  </div>
</div>

<div class="col-xs-12 col-md-4">
<div class="form-group">
   <?php echo InputGeneral("text", "dp_102_mail", "form-control", "dpmail_llamado", "escriba el correo electronico", "Correo Electrónico:",""); ?>
  </div>
</div>
</div>
</div>
...
<div class="form-group">
  ...
  <input type="number" class="form-control" id="muestra_edad" name="dp_edad">
</div>
  </div>
  </div>
...
<div class="checkbox">
  <label>
<input name="ds_discapacidad" type="checkbox" value="1"> Es una persona discapacitada 
</label>
</div>
   </div>
  </div>
...
<div class="form-group">
  ...
  <div class="col-xs-12 col-md-6">
      ...
      <div class="form-group">
  <label class="control-label">Edad</label>
  <input type="text" class="form-control" id="muestra_edada" disabled="disabled">
</div>
  </div>
  </div>
...
<div class="checkbox">
  <label>
<input name="dpa_discapacidad" type="checkbox" value="1"> Es una persona discapacitada 
</label>
</div>
   </div>
...
<label>Tipo de Violencia</label>
  <?php
  $q_text = "select * from tb_tipo_violencia where tv_tipo = 1";
...
<div class="form-group">
<label>Ambito de Violencia</label>
<?php
  $q_text = "select * from tb_tipo_violencia where tv_tipo = 0";
  $q_query = mysql_query($q_text);
    while($dat1 = mysql_fetch_array($q_query)){
      ?>
  <label class="checkbox-inline">
  <input type="checkbox" name="<?php echo 'tv'.$dat1['tv_id']; ?>" value="<?php echo $dat1['tv_id']; ?>"> <?php echo utf8_encode($dat1['tv_name']); ?> | 
</label>
<?php
}
?>
...
<div id="dataderivaciones" class="collapse">
  ...
  <?php
  $q_text = "select * from tb_derivaciones_102 where der_es = 0";
  $q_query = mysql_query($q_text);
    while($dat = mysql_fetch_array($q_query)){
      ?>
  <label class="checkbox-inline">
...
<br clear="all"><br>
<div class="form-group">
<label>Guardia de Primeras Horas</label>
  
  <label class="checkbox-inline">
  <input type="checkbox" name="guardia" value="si"> Guardia
</label>
...
<div class="panel panel-info">
  <div class="panel-heading">
  <h3 class="panel-title" data-toggle="collapse" data-target="#dataresguardo">
  <span class="glyphicon glyphicon-home"></span>  Resguardo <span class="caret"></span></div>
  </h3>
</div>
<div id="dataresguardo" class="collapse">

  <?php
  $q_text = "select * from tb_102_resguardo";
  $q_query = mysql_query($q_text);
    while($dat_r = mysql_fetch_array($q_query)){
      ?>
  <label class="checkbox-inline">
  <input type="checkbox" name="<?php echo 'res_'.$dat_r['res_id']; ?>" value="<?php echo $dat_r['res_id']; ?>"> <?php echo utf8_encode($dat_r['res_name']); ?> | 
</label>
<?php
}
?>
      ...
      $("#frecuencia").change(function(){
       ...
       $("#idtiporelacion").change(function(){
        if($("#idtiporelacion").val() == 1){
              $("#datos_quien_llama").hide();
              $("#datos_instituciones").hide();
        } 
       
        if($("#idtiporelacion").val() == 2){
              $("#datos_quien_llama").show();
              $("#datos_instituciones").hide();
        } 
        if($("#idtiporelacion").val() == 3){
              $("#datos_quien_llama").hide();
              $("#datos_instituciones").show();
        } 
      });
...
//   $("#listinstituciones").click(function(){
        $.post("tools/listado_instituciones.php", function(dataselect){
              $("#listinstituciones").html(dataselect);
      });
 //  });
        $("#listinstituciones").change (function(){
          var valor = $("#listinstituciones").val();
          if(valor == 3 || valor == 5 || valor == 6 || valor == 7  || valor == 10  || valor == 11 || valor == 14 || valor == 15){
            switch(valor){
              case '3':
              var valor_input = "Comisaria Nro ";
              break;

              case '5':
              var valor_input = "Fiscalia Nro ";
              break;

              case '6':
              var valor_input = "Juzgado de Familia Nro ";
              break;

              case '7':
              var valor_input = "Juzgado de Paz Nro ";
              break;

              case '10':
              var valor_input = "Centro de Salud ";
              break;

              case '11':
              var valor_input = "SAT Nro ";
              break;

              case '14':
              var valor_input = "CAAT Nro ";
              break;

            }
            $("#nuevainstitucion").val(valor_input);
              $("#agrega_instituciones").show();
                        } else {
              $("#agrega_instituciones").hide();
              $("#nuevainstitucion").val("");            
                        }
        });

        $("#agrega_ins").click(function(){
          var add_valor = $("#nuevainstitucion").val();
   /*        $.post("tools/nueva_institucion.php",{nuevo_val: add_valor} , function(dataagrega){
              alert(dataagrega);
      }); */
           $.post("tools/listado_instituciones.php", {nuevo_val: add_valor}, function(dataselect){
              $("#listinstituciones").html(dataselect);
               $("#agrega_instituciones").hide();
              $("#nuevainstitucion").val("");  
      });
