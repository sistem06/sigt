...
<body>
  ...
  <div class="container">
  <h3>Agrupando la jerarquia <?php echo BuscaRegistro ("tb_categorias", "cat_id", $_GET['cat_id'], "cat_name"); ?></h3>
   ...
   <laber for="seagrupa">Agrupa Con: </laber>
      <select class="form-control" name="seagrupa" id="seagrupa">
          <?php
            $txt = "select * from tb_categorias where (cat_id != '".$_GET['cat_id']."') order by cat_name";
            $query = mysql_query($txt);
                while($dat = mysql_fetch_array($query)){
                  echo '<option value="'.$dat['cat_id'].'">'.utf8_encode($dat['cat_name']).'</option>';
                }
          ?>
      </select>
